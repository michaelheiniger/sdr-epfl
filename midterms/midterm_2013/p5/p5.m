...
%     [R, ~] = xcorr(received_corrected, training);
...
% end
 ...
 [R, ~] = xcorr(received, training);
 R = R(length(received):end);
 [corr, tau] = max(abs(R));
tr_rx = received(tau:tau+length(training)-1);
phase_diff = phase(sum(tr_rx.*conj(training)));

% Correct for rotation
received_rot_corrected = received.*exp(-1j*phase_diff);
