...
% Verify that x contains only elements between 0 and M-1
if ~all((x >= 0) & (x < M) & (x == fix(x)))
    error('my_modulator:wrongInputValues', 'Elements of input X must be integers in the range [0, M-1]');
end

mapping = mapping(:);
if (size(x, 1) == 1) % input is row vector
    mapping = transpose(mapping); % we need to index with a row vector to obtain a row vector
end
