...
preamble = randi([0,3], [num_useful_carriers, 1]);
...
rx_signal = 0.5*conv(tx_signal, h);

% add AWGN
sigma = 10^(-SNR/20) * sqrt(E_tx);
noise = sigma/sqrt(2)* (randn(size(rx_signal)) + 1i*randn(size(rx_signal)));

rx_signal_noisy = 0.1*(rx_signal + noise);

% Receiver
Rf = ofdm_rx_frame(rx_signal_noisy, num_carriers, num_zeros, prefix_length);
...
% h known
lambda1 = channel_est1(num_carriers, num_zeros, h);
...
figure(); plot(suf_data3, '*b'); xlabel('Re'); ylabel('Im'); title('MMSE');
hold on; plot(constel_data, '+r'); grid on; legend('Received constellation', 'Transmitted constellation');
axis([-5,5,-5,5]);
...
figure(); plot(suf_data4, '*b'); xlabel('Re'); ylabel('Im'); title('Division');
hold on; plot(constel_data, '+r'); grid on; legend('Received constellation', 'Transmitted constellation');
axis([-5,5,-5,5]);

% debugging information
% suf_data5=Rf(length(preamble_symbols)+1:end);
% figure(); plot(suf_data5, '*b'); xlabel('Re'); ylabel('Im'); title('No equalization');
% hold on; plot(constel_data, '+r'); grid on; legend('Received constellation', 'Transmitted constellation');
% axis([-5,5,-5,5]);
